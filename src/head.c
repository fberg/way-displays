...
wl_fixed_t get_fixed_scale(double scale) {
...
int32_t get_scaled_length(int32_t length, wl_fixed_t fixed_scale) {
	// scales a (pixel) length by fixed_scale

	// TODO: this only works on new Sway (with fractional scaling, see comment in `get_fixed_scale()`)
	//  It breaks computed output heights/widths on Sway 1.8.1.
	wl_fixed_t adjusted_scale = (double)fixed_scale / 256 * 120 + 0.5;

	return floor((double)length * 120 / adjusted_scale);
}

wl_fixed_t head_auto_scale(struct Head *head, double min, double max) {
	if (!head || !head->desired.mode) {
		return get_fixed_scale(1.0);
	}
	...
	return get_fixed_scale((double) dpi_quantized / 96);
}
...
void head_scaled_dimensions(struct Head *head) {
	...
	head->scaled.height = get_scaled_length(head->scaled.height, head->desired.scale);
	head->scaled.width = get_scaled_length(head->scaled.width, head->desired.scale);
}
